...
# TODO:使用除了tf.metrics之外的评价方法，并使其能再tensorboard里显示

# TODO:需要把一切都流程化，从生数据一键跑完。
...
TIME_STEP = 15
DATA_PATH = "../../data/Train&Test/C1S4_newlabel_hybrid_timestep15_" + VALIDATE_Y_DATE + "_regression/"
...
MODEL_DIR = "../../data/models/hybrid_regression_"
...
def result_analyze(predictions, y_validate, eval_result, opt):
    ...
    # 对超额收益的预测值排序，并选取出其中的所有超额收益为正的股票进行购买，计算收益率
    preds_origin = []
    preds = []
    pos_preds = []
    neg_preds = []
    for pred in predictions_dict["logits"]:
        preds_origin.append(float(pred))
        preds.append(float(pred))
        if float(pred) >= 0.0000001:
            pos_preds.append(float(pred))
        else:
            neg_preds.append(float(pred))
    preds.sort(reverse=True)
    pos_preds.sort(reverse=True)
    neg_preds.sort(reverse=True)
    print("预测出收益为正的数量：", len(pos_preds))
    print("预测出收益为负的数量：", len(neg_preds))
    ...
    # 选出预测值大于0的股票列表
    select_ts_code = list(valid_set_pre[valid_set_pre["pre_y"] >= 0.0000001]["ts_code"])
    # 所有选出的股票的收益值，为分子
    all_change = 0
    ...
    try:
        ...
        # 所有股票的收益率计算方法就是将所有单支股票的收益率加起来即可
        final_shouyilv = all_change / s_date_2_close_sum
    except Exception as e:
        final_shouyilv = "没有推荐出任何可购入的股票！"
    print("最终收益率：", final_shouyilv)
    print("预测结果（按样本顺序）：", preds_origin)

    # 计算MSE等评价指标
    preds_origin = np.array(preds_origin).reshape(-1, 1)
    mse = mean_squared_error(y_validate, preds_origin)

    # 保存实验结果
    result_file = open(TARGET_PATH + "Experiment_result_regression", "w")
    result_file.write(time.strftime('%Y.%m.%d', time.localtime(time.time())) + "\n")
    result_file.write("预测出收益为正的数量：" + str(len(pos_preds)) + "\n")
    result_file.write("预测出收益为负的数量：" + str(len(neg_preds)) + "\n")
    result_file.write("最终收益率：" + str(final_shouyilv) + "\n")
    result_file.write("预测结果（按样本顺序）：" + str(preds_origin) + "\n")
    result_file.write('Test set mse:' + str(mse) + "\n")
    result_file.close()

    # 保存预测结果，将y_pred和label一起保存。
    # ndarray横向拼接
    preds_origin = np.array(preds_origin).reshape(-1, 1)
    super_validate_data_right = np.hstack([preds_origin, y_validate])
    super_validate_data_right = pd.DataFrame(super_validate_data_right)
    super_validate_data_right.to_csv(TARGET_PATH + "super_validate_data_right.csv", index=False, index_label=False)
    print("super_validate_data_right saved!")
    return None
